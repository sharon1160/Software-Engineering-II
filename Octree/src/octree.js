...
class Point {
  constructor(x, y, z) {
    ...
    // Guardamos la esfera en dicha posicion y la agregamos a la escena
...
class Box {
  ...
  contains(point) { // devuelve un booleano si se encuentra un punto dentro del octree o no
  ...
  intersects(box) { // devuelve un booleano que indica si dos cuadrantes se intersectan
...
class Octree {
  constructor(box, n) {
    this.box = box;
    this.capacity = n; // numero de puntos por cuadrante
    this.points = []; // arreglo de puntos
    this.divided = false; // booleano que indica si esta dividido o no

    this.color = new THREE.Color(Math.random() * 1, Math.random() * 1, Math.random() * 1); // color random
    ...
    // lo coloca en dicha posicion y lo coloca en escena
